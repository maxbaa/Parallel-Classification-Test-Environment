...
class CascadeSVC(ClassifierMixin, BaseEstimator):
    ...
    def _kfold_svc(self, id: np.ndarray, X: np.ndarray, y: np.ndarray)\
        ...
        all support vectors"""
        n_splits = max(2, round(X.shape[0] / self.fold_size))
        folds = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=self.random_state)
        idsv, Xsv, ysv = [], [], []
        ...
        idsv, Xsv, ysv = np.hstack(idsv), np.vstack(Xsv), np.hstack(ysv)
        return idsv, Xsv, ysv
